...
###############################################################
#                                                             #
#    Translated Kéry (2010) code for JAGS                     #
#                                                             #
###############################################################

# This is the translation of the code in the book by Kéry, 2010,
# into JAGS. JAGS, WinBUGS, OpenBUGS, and to some degree also 
# NIMBLE all use very similar dialects of the BUGS language
# There are multiple R/JAGS interfaces, and here we use jagsUI.

# Last changes: 27 March 2019

# Currently, all the chapters until and including 12 have 
# been translated. The remaining chapters wait to be translated ...
...
library(jagsUI)		                # Load jagsUI

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(mass = y1000, nobs = length(y1000)))
...
# MCMC settings
na <- 1000              # Number of adaptation steps
...
out <- jags(data = bdata, inits = inits, parameters.to.save = params, model.file = "model.txt", 
n.adapt = na, n.thin = nt, n.chains = nc, n.burnin = nb, n.iter = ni, parallel = FALSE)  # Can try out  parallel = TRUE
...
###### Next 8 lines of code would need to be adapted to the different output format of JAGS
par(mfrow = c(3,1))
matplot(out$samples[1:999,1:3,1]), type = "l")
...
par(mfrow = c(3,1))                                           # This works again in JAGS
...
### 7.1.4. Analysis using JAGS


# Bundle and summarize the data set passed to JAGS
str(bdata <- list("x", "y", "n"))    # Works, but less informative
str(bdata <- list(x = x, y = y, n = n))  # More informative
...
# MCMC settings
na <- 1000  ;  nc <- 3	;  ni <- 1000  ;  nb <- 1  ;  nt <- 1

# Call JAGS from R, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "ttest.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 2))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 7.2.4. Analysis using JAGS

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(y1 = y1, y2 = y2, n1 = n1, n2 = n2))

# Specify model in BUGS language
cat(file = "h.ttest.txt", "
model {
...
# MCMC settings
na <- 1000  ;  nc <- 3	;  ni <- 2000  ;  nb <- 500  ;  nt <- 1

# Unleash the raw power of JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "h.ttest.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 2))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 8.4. Analysis using JAGS


### 8.4.1. Fitting the model

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(x = x, y = y, n = n))
...
out <- jags(bdata, inits, params, "linreg.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 9.2.3. Bayesian analysis using JAGS
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "anova.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 9.3.3. Bayesian analysis using JAGS
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "re.anova.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 10.5. Analysis using JAGS


### 10.5.1. Main-effects ANOVA using JAGS

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(wing=wing, elev = as.numeric(elev), pop = as.numeric(pop), n = length(wing)))


# Specify model in BUGS language
cat(file = "2w.anova.txt", "
model {
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "2w.anova.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
summary(out)             # Other summary
View(out)                # Useful for posterior summaries when very many params
summary(mainfit)
...
# Bundle and summarize the data set passed to JAGS
str(bdata <- list(wing=wing, elev = as.numeric(elev), pop = as.numeric(pop), 
n = length(wing), n.elev = length(unique(elev)), n.pop = length(unique(pop))))

# Specify model in BUGS language
cat(file = "2w2.anova.txt", "
model {
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "2w2.anova.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 11.4. Analysis using JAGS (and a cautionary tale 
...
# Parameters to estimate
params <- c("alpha", "beta", "sigma", "a.effe2", "a.effe3", 
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "lm.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 12.3.2. Bayesian analysis using JAGS
...
# MCMC settings
na <- 1000  ;  nc <- 3  ;  ni <- 2000  ;  nb <- 500  ;  nt <- 2

# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "lme.model1.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 12.4.2. Bayesian analysis using JAGS
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "lme.model2.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
# Parameters to estimate
params <- c("alpha", "beta", "mu.int", "sigma.int", "mu.slope", 
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "lme.model3.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
...
######### Until here, stuff translated from analysis in WinBUGS to analysis in JAGS
# Below, original version of the code for WinBUGS
# Translation should be easy though.....
...
### 13.5. Analysis using JAGS

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(C = C, x = as.numeric(x)-1, n = length(x)))

# Specify model in BUGS language
cat(file = "Poisson.t.test.txt", "
model {
...
# MCMC settings
na <- 1000  ;  nc <- 3  ;  ni <- 3000  ;  nb <- 1000  ;  nt <- 2

# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "Poisson.t.test.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
###############################################################
#  Chapter 14: Overdispersion, zero-inflation and offsets     #
###############################################################
...
### 14.1.4. Analysis using JAGS

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(C.OD = C.OD, x = as.numeric(x)-1, n = length(x)))

# Specify model in BUGS language
cat(file = "Poisson.OD.t.test.txt", "
model {
# Priors
 alpha ~ dnorm(0,0.001)
 beta ~ dnorm(0,0.001)
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "Poisson.OD.t.test.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "ZIP.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 14.3.4. Analysis using JAGS


# Bundle and summarize the data set passed to JAGS
str(bdata <- list(C = C, x = as.numeric(x)-1, logA = log(A), n = length(x)))
...
# MCMC settings
na <- 1000  ;  nc <- 3  ;  ni <- 1100  ;  nb <- 100  ;  nt <- 2

# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "Offset.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 15.4. Analysis using JAGS

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(C = C, pop = as.numeric(pop), n.groups = n.groups, length = length, n = n))
...
# MCMC settings
na <- 1000  ;  nc <- 3  ;  ni <- 4500  ;  nb <- 1500  ;  nt <- 5
...
### 16.3.2. Analysis using JAGS

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(C = C, pop = as.numeric(pop), year = year, n.groups = n.groups, n = n))
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "glmm.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 17.4. Analysis using JAGS

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(C = C, N = 50, species = c(0,1), n = length(C)))

# Specify model in BUGS language
cat(file = "Binomial.t.test.txt","
model {
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "Binomial.t.test.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(2, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
# MCMC settings
na <- 1000  ;  nc <- 3  ;  ni <- 1500  ;  nb <- 500  ;  nt <- 5
...
### 19.4.2. Analysis using JAGS

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(C = C, N = N, pop = as.numeric(pop), prec = prec, 
...
### 20.3. Analysis using JASG

# Bundle and summarize the data set passed to JAGS
str(bdata <- list(y = y, humidity = humidity, R = dim(y)[1], T = dim(y)[2]))
...
# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "model.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
### 21.3. Analysis using JAGS

# Bundle and summarize the data set passed to JAGS
R = dim(y)[1]
n = dim(y)[1] * dim(y)[2]
vege2 = (vege * vege)
str(bdata <- list(R = R, vege = vege, vege2 = vege2, n = n, C = C, site.p = site.p, vege.p = vege.p))
...
# MCMC settings
na <- 1000  ;  nc <- 3  ;  ni <- 40000  ;  nb <- 20000  ;  nt <- 20   # Longer chains than in book

###### A note on the MCMC settings for this model:
# Depending on the particular data set you generated (which depends on the seed)
# used in the random number generators (you can initialize those by doing 'set.seed(x)',
# with x an integer of your choice), you may have more or less trouble getting the BinMix model
# to convergence
# The above settings should sometimes be enough, but not always .... but, well, I just see (reading on, below)
# that bad convergence was really the point of this particular model run, oh well ....

# Call JAGS, check convergence and summarize posteriors
out <- jags(bdata, inits, params, "BinMix.txt", n.adapt = na, n.thin = nt, n.chains = nc, 
n.burnin = nb, n.iter = ni, parallel = TRUE)
par(mfrow = c(3, 3))  ;  traceplot(out)  ;  par(mfrow = c(1, 1))
print(out, dig = 3)
...
# MCMC settings
na <- 1000  ;  nc <- 3  ;  ni <- 60000  ;  nb <- 10000  ;  nt <- 50
# Took more than 20 min with WinBUGS and on my old laptop from back in the days when I wrote that book :)
# Now, with JAGS and on my not superfast desktop, it takes less than 5 minutes
